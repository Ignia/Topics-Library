...
namespace OnTopic {
  ...
  public class Topic {
    ...
    /// </param>
    /// <returns>Returns <c>true</c> if any elements are dirty, otherwise <c>false</c>.</returns>
    ...
    /// <summary>
    ///   Determines if the topic is dirty, and provides an output <see cref="DirtyObjects"/> enum specifying what elements
    ///   are dirty.
    /// </summary>
    /// <param name="isDirty">
    ///   An <see cref="DirtyObjects"/> enum which tracks which items are dirty.
    /// </param>
    /// <returns>Returns <c>true</c> if any elements are dirty, otherwise <c>false</c>.</returns>
    public bool IsDirty(out DirtyObjects isDirty) {
      isDirty = DirtyObjects.None;
      if (_isDirty) {
        isDirty |= DirtyObjects.Topic;
      }
      if (Attributes.IsDirty(true)) {
        isDirty |= DirtyObjects.Attributes;
      }
      if (References.IsDirty) {
        isDirty |= DirtyObjects.References;
      }
      if (Relationships.IsDirty()) {
        isDirty |= DirtyObjects.Relationships;
      }
      return _isDirty;
    }
